...
from .serializers import MenteeDashboardSerializer, MentorDashboardSerializer, MentorInfoSerializer, MeetingScheduleSerializer, WeeklyCheckInSerializer
...
class MentorDashboardView(APIView):
    ...
    def get(self, request):
        ...
        last_schedule = MeetingSchedule.objects.filter(
        ...
        next_checkin = WeeklyCheckIn.objects.filter(
            loop__mentor=mentor_profile, mentor_checked_in=False, status=WeeklyCheckIn.STATUS_PENDING,
        ...
        next_meeting = MeetingSchedule.objects.filter(
